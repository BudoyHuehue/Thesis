...
//[main]
double gmax_rcga = 600;
double gmax_afsa = 0;
//[39]
double b_mutation = 0.5;
// P_s = zeros()
// P_h =zeros()
...
double sum_upstream(int pop,int t,int j){
	...
	for(int i=0;i<R_u[j];i++){
		if((t-time_delay[i])<0){
			...
			if(isinf(population[pop][t-time_delay[i]][j])){
...
/* deque<double> P_scF(int index, int t){//
	deque<double> temp;
	for(int ns=N_h;ns<N_h+N_s;ns++){
		if(t==0) temp.push_back(0);
		else{
			temp.push_back(population[index][t][ns] - population[index][t][ns-1]);
		}
	}
	return temp;
} */
...
double P_hsum(int index, int t){//work on the equation
	double temp=0;
	for(int a = 0; a<N_h;a++){
		temp = temp + c1[a]*pow(volume[index][t][a],2) + c2[a]*pow(population[index][t][a],2) + c3[a]*volume[index][t][a]*population[index][t][a] + c4[a]*volume[index][t][a] + c5[a]*population[index][t][a] + c6[a];
	}
	//cout<<index<<" "<<t<<endl;
...
void generate_bulletin(){
	//bubble sort to get best individual
	evaluation();//evaluated and bubble sorted
	bulletin = 0;
}

void update_bulletin(){
	evaluation();//evaluated and bubble sorted
	bulletin = 0;
}
...
double evaluation(deque<deque<double> > temp1, int index){
	double temp=0;
	for(int t=0;t<T;t++){
		for(int i=0;i<N_h+N_s;i++){
			temp = temp + a_si + b_si*temp1[t][N_h+i] + c_si*pow(temp1[t][N_h+i],2) + fabs(d_si*sin(e_si*(P_smin - temp1[t][N_h+i])));
...
cout<<"=================================================================="<<endl;
for(int a=0;a<POP_SIZE;a++){//printing
		cout<<"Individual: "<<a<<endl;
		for(int t=0;t<T;t++){
			for(int j=0;j<N_h+N_s;j++){
				cout<<population[a][t][j]<<" ";
			}
			cout<<endl;
		}
		cout<<endl;
...
//cout<<"Check1"<<endl;
/* for(int i=0;i<volume.size();i++){//printing of volume water balance
	 for(int t=0;t<volume[i].size();t++){
		for(int j=0;j<volume[i][j].size();j++){
			cout<<volume[i][t][j]<<" ";
		}
		cout<<endl;
	} 
...
/* 	cout<<"------------------------------------------------------------"<<endl; */
	...
	//cout<<"Best: "<<population_fitness[0]<<endl;
	//cout<<"Worst: "<<population_fitness[99]<<endl;
